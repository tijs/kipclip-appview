...
| `deploy/release/tap-update.sh`                         | Build flow: stage downstream patch copies into a private tap-readable mktemp directory → re-assert tap:tap ownership + owner-writability on the dedicated build tree (a root-owned checkout makes git refuse with "dubious ownership" — no safe.directory) → fetch → hard-reset + clean the dedicated build tree to the immutable base sha (a previous tick's applied patch never survives into the next) → `git apply --check` + apply staged patches (fail loud on drift) → build → record `.version` (source sha) + `.patches.sha256` + `.binary.sha256` → restart → health check → rollback restoring binary AND recorded SHAs. |
